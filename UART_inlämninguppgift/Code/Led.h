...
//Definerar de olika lägena som LED kan befinna sig i
typedef enum {
  OFF = 0,
  ON = 1
}LedState_Type;

// Definerar klassen Led, med atrribut color, state
class Led{

  private:
      LedColor_Type color; // Attribut för att hålla reda på LED-färgen
      LedState_Type state; // Attrbiut för att hålla reda på LED-status

  public:
      //konstruktor för klassen Led
      Led(LedColor_Type _color,LedState_Type _state);
      void setState(LedState_Type _state);// Metod för att ställa in LED-status
      LedState_Type getState() const;//Metod för att hämta LED-status
